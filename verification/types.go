...
// GetVerificationLevel finds if the given input corresponds to a supported VerificationLevel, otherwise throws an error
...
func parseCustomVerification(input interface{}) (*VerificationLevel, error) {
	...
	if baseLevel == nil {
		return nil, fmt.Errorf("invalid custom signature verification %q", cvl["level"])
	}
	if baseLevel == Skip {
	...
	// override the verification actions with the user configured actions
